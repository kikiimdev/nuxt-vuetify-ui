  ...
  <KBlock>
    ...
    <NuxtErrorBoundary>
      <slot name="table" :items="items" :total="total">
        <VDataTableServer :headers="headers" :loading="pending" :items="items" :items-length="total"
          :items-per-page="options.itemsPerPage" :class="tableClass" @update:options="updateOptions($event)">
          <template v-for="header of headers" #[`item.${header.key}`]="{ item }">
            <slot :key="`item.${header.key}`" :name="`item.${header.key}`" :item="item" :header="header">
...
const items = computed(() => {
  const selectedData = (props.dataKey && data.value) ? data.value[props.dataKey] : data.value
  if (!selectedData) {
    return []
  }
